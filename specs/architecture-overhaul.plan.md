...
names; the adapter's `getDerivedState`/`resolveTenant` read as `toState`-family
...
| 8 — tidy periphery | ◑ in progress (8a, 8b done) | 8a `127ec27`; 8b `a779dc6` | 403 pass (25 files) | **Sub-committed** (4 move-groups, ~28 importers — too big for one commit). **8a done:** `rosterGenerator/` → top-level `src/generation/`. **8b done:** the 3 genuine read-model files (`rosterStats`, `availabilityUtils`, `distributionUtils` + tests) → `src/readmodel/` (git mv); their `../schema|state|rules|evaluation` imports were already single-`..` (same depth as `utils/`) so unchanged; only `distributionUtils`'s two intra-`utils` siblings (`statsTheme`, `colorUtils`) became `../utils/…`; rewired 4 external importers (App.jsx ×2, RosterStatsPanel, **QualityMetrics** — nearly missed). **Discovery:** `statsTheme.js` is **misnamed** — it's the app-wide *design-system* token module (used by ~12 components, has `designSystem.lint.test.js`, owned by [design-system.md](design-system.md)), **not** a stat. It does **not** go to `readmodel/`; renaming `statsTheme`→`designSystem` + rehoming it is a separate design-system concern, flagged for 8c/9. **Remaining:** 8c generic → `lib/` (`calendarUtils`/`colorUtils`/`dataExport`/`bulkClear`) + resolve `statsTheme`; 8d `telegram.js` → `integrations/`. |
