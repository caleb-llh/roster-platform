...
| [architecture-overhaul.plan.md](architecture-overhaul.plan.md) | **DESIGN (not built).** Target layer model for a future refactor: the rule-engine framing (Schema vocabulary; Rules ⋈ State → Evaluation; Generation mutates State), why Schema is orthogonal to both Rules and State, the periphery (presentation/storage/integrations) + cross-cutting authorization & lib, the target folder layout, naming decisions, and a green-per-step refactor order. Links to the owning behaviour specs rather than restating them. |
